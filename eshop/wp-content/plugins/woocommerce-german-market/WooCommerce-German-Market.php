...
class Woocommerce_German_Market {
	...
	public function __construct () {
		...
		add_filter( 'woocommerce_review_order_after_submit',					array( 'WGM_Template', 'add_review_order' ) );
		...
		add_filter( 'woocommerce_order_product_title',							array( 'WGM_Template', 'add_shipping_time_to_product_title' ), 10, 2 );
		add_filter( 'woocommerce_order_button_text',							array( 'WGM_Template', 'change_order_button_text' ), 5, 1 );
		add_filter( 'woocommerce_after_checkout_validation',					array( 'WGM_Template', 'do_de_checkout_after_validation' ), 5, 1 );
		...
		if( Woocommerce_German_Market::is_wc_2() )
			add_filter( 'woocommerce_review_order_before_order_total',			array( 'WGM_Template', 'add_items_on_review_order' ) );
		else
			add_filter( 'woocommerce_before_order_total ',						array( 'WGM_Template', 'add_items_on_review_order' ) );

		add_filter( 'woocommerce_get_order_item_totals',						array( 'WGM_Template', 'add_items_order_table' ), 10, 2 );
		...
		add_filter( 'woocommerce_checkout_item_subtotal', 						array( 'WGM_Template', 'add_mwst_rate_to_product_item' ), 10 ,3 );
		...
		add_filter( 'woocommerce_order_table_item_quantity',					array( 'WGM_Template', 'add_product_short_desc_to_order_title' ), 10, 2 );
		add_filter( 'woocommerce_checkout_item_quantity',						array( 'WGM_Template', 'add_product_short_desc_to_checkout_title' ), 10, 2 );
		add_filter( 'woocommerce_single_product_summary',						array( 'WGM_Template', 'show_extra_costs_eu' ) , 11 );
		add_filter( 'woocommerce_available_shipping_methods', 					array( 'WGM_Template', 'hide_standard_shipping_when_free_is_available' ) , 10 );

		add_filter( 'woocommerce_order_actions',                                array( 'WGM_Helper', 'deactivate_pay_for_order_button' ) );
		add_filter( 'wp_head',													array( 'WGM_Helper', 'add_checkout_body_classes' ) );

		add_filter( 'woocommerce_init',											array( 'WGM_Cash_On_Delivery', 'remove_standard_cod' ) );
	...
	public static function load_styles() {
		...
		if ( is_admin() ) {
			...
			if( get_option( WGM_Helper::get_wgm_option( 'woocommerce_options_installed' ) ) !== 1 )  {
				wp_register_style( 'woocommerce-de-activation-style', plugins_url( 'css/activation.css', __FILE__ ) );
		...
		// Frontend styles
			...
			if ( get_option( 'load_woocommerce-de_standard_css' ) === 'on' ) {
				wp_register_style( 'woocommerce-de_frontend_styles', plugins_url( 'css/frontend.css', __FILE__ ), array(), '1.0' );
	...
	*/
	public static function enqueue_admin_scripts() {
		wp_enqueue_script( 'woocommerce_de_admin', plugin_dir_url( __FILE__ ) . 'js/WooCommerce-German-Market-Admin.js', array( 'jquery', 'woocommerce_admin' ) );
	...
	*/
	public static function enqueue_frontend_scripts() {
		...
		wp_enqueue_script( 'woocommerce_de_frontend', plugin_dir_url( __FILE__ ) . 'js/WooCommerce-German-Market-Frontend.js', array( 'jquery', 'woocommerce' ) );
	...
	*/
	...
	/**
	 * Check if current WooCommerce Version is equal or above 2.0
	 *
	 * @since	1.1.5beta
	 * @author	jj
	 * @access	public
	...
	public static function is_wc_2() {
		...
		return version_compare( @$woocommerce->version , '2.0', '>=' );
